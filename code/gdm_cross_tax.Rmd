...
# rcp scenarios
covars_rcp <- c("covars_rcp26_2030", "covars_rcp26_2050", "covars_rcp26_2070", "covars_rcp85_2030", "covars_rcp85_2050", "covars_rcp85_2070")

# function to calculate compositional change due to climate change
cc_df <- function(gdmr_tax, scenarios) {
  
  ltime <- lapply(scenarios, function(s) {
    
    # predict
  timep <- predict(object = gdmr_tax, data = raster::stack(covars), time = TRUE, predRasts = raster::stack(get(s)))
...
# run compositional change function
cc_plants <- cc_df(gdmr_tax = gdmr_plants, scenarios = covars_rcp)

# only RCP 2.6
cc_plants_rcp26 <- dplyr::filter(cc_plants, scenario %in% covars_rcp[1:3]) %>% 
  dplyr::mutate(scenario = dplyr::case_match(scenario, "covars_rcp26_2030" ~ "2030", "covars_rcp26_2050" ~ "2050", "covars_rcp26_2070" ~ "2070"))

# interquartile range
cc_plants_rcp26_iqr <- cc_plants_rcp26 %>% 
  dplyr::group_by(scenario) %>% 
  dplyr::summarise(Q1 = quantile(change, 0.25, na.rm = TRUE),
                   Q3 = quantile(change, 0.75, na.rm = TRUE))

# only RCP 8.5
cc_plants_rcp85 <- dplyr::filter(cc_plants, scenario %in% covars_rcp[4:6]) %>% 
  dplyr::mutate(scenario = dplyr::case_match(scenario, "covars_rcp85_2030" ~ "2030", "covars_rcp85_2050" ~ "2050", "covars_rcp85_2070" ~ "2070"))

# interquartile range
cc_plants_rcp85_iqr <- cc_plants_rcp85 %>% 
  dplyr::group_by(scenario) %>% 
  dplyr::summarise(Q1 = quantile(change, 0.25, na.rm = TRUE),
                   Q3 = quantile(change, 0.75, na.rm = TRUE))
...
# run compositional change function
cc_butterflies <- cc_df(gdmr_tax = gdmr_butterflies, scenarios = covars_rcp)

# only RCP 2.6
cc_butterflies_rcp26 <- dplyr::filter(cc_butterflies, scenario %in% covars_rcp[1:3]) %>% 
  dplyr::mutate(scenario = dplyr::case_match(scenario, "covars_rcp26_2030" ~ "2030", "covars_rcp26_2050" ~ "2050", "covars_rcp26_2070" ~ "2070"))

# interquartile range
cc_butterflies_rcp26_iqr <- cc_butterflies_rcp26 %>% 
  dplyr::group_by(scenario) %>% 
  dplyr::summarise(Q1 = quantile(change, 0.25, na.rm = TRUE),
                   Q3 = quantile(change, 0.75, na.rm = TRUE))

# only RCP 8.5
cc_butterflies_rcp85 <- dplyr::filter(cc_butterflies, scenario %in% covars_rcp[4:6]) %>% 
  dplyr::mutate(scenario = dplyr::case_match(scenario, "covars_rcp85_2030" ~ "2030", "covars_rcp85_2050" ~ "2050", "covars_rcp85_2070" ~ "2070"))

# interquartile range
cc_butterflies_rcp85_iqr <- cc_butterflies_rcp85 %>% 
  dplyr::group_by(scenario) %>% 
  dplyr::summarise(Q1 = quantile(change, 0.25, na.rm = TRUE),
                   Q3 = quantile(change, 0.75, na.rm = TRUE))
...
# run compositional change function
cc_birds <- cc_df(gdmr_tax = gdmr_birds, scenarios = covars_rcp)

# only RCP 2.6
cc_birds_rcp26 <- dplyr::filter(cc_birds, scenario %in% covars_rcp[1:3]) %>% 
  dplyr::mutate(scenario = dplyr::case_match(scenario, "covars_rcp26_2030" ~ "2030", "covars_rcp26_2050" ~ "2050", "covars_rcp26_2070" ~ "2070"))

# interquartile range
cc_birds_rcp26_iqr <- cc_birds_rcp26 %>% 
  dplyr::group_by(scenario) %>% 
  dplyr::summarise(Q1 = quantile(change, 0.25, na.rm = TRUE),
                   Q3 = quantile(change, 0.75, na.rm = TRUE))

# only RCP 8.5
cc_birds_rcp85 <- dplyr::filter(cc_birds, scenario %in% covars_rcp[4:6]) %>% 
  dplyr::mutate(scenario = dplyr::case_match(scenario, "covars_rcp85_2030" ~ "2030", "covars_rcp85_2050" ~ "2050", "covars_rcp85_2070" ~ "2070"))

# interquartile range
cc_birds_rcp85_iqr <- cc_birds_rcp85 %>% 
  dplyr::group_by(scenario) %>% 
  dplyr::summarise(Q1 = quantile(change, 0.25, na.rm = TRUE),
                   Q3 = quantile(change, 0.75, na.rm = TRUE))
...
# plants violin plot
vio_plants <- ggplot2::ggplot() +
  # rcp 2.6 violin
  gghalves::geom_half_violin(data = cc_plants_rcp26, ggplot2::aes(x = scenario, y = change), colour = "#998ec3", fill = "#998ec3", alpha = 0.5, position = ggplot2::position_nudge(x = -nudge)) +
  # rcp 8.5 violin
  gghalves::geom_half_violin(data = cc_plants_rcp85, ggplot2::aes(x = scenario, y = change), side = "r", colour = "#f1a340", fill = "#f1a340", alpha = 0.5, position = ggplot2::position_nudge(x = nudge)) +
  # rcp 2.6 median
  ggplot2::stat_summary(data = cc_plants_rcp26, ggplot2::aes(x = scenario, y = change), fun = median, geom = "point", colour = "#69608C", position = ggplot2::position_nudge(x = -nudge)) +
  # rcp 2.6 label
  ggplot2::stat_summary(data = cc_plants_rcp26, ggplot2::aes(x = scenario, y = change, label = paste0(round(..y.., 0), "%")), fun = median, geom = "text", colour = "#69608C", hjust = rcp26_h) +
  # rcp 2.6 iqr
  ggplot2::geom_errorbar(data = cc_plants_rcp26_iqr, ggplot2::aes(x = scenario, ymin = Q1, ymax = Q3), width = 0, colour = "#69608C", position = ggplot2::position_nudge(x = -nudge)) +
  # rcp 8.5 median
  ggplot2::stat_summary(data = cc_plants_rcp85, ggplot2::aes(x = scenario, y = change), fun = median, geom = "point", colour = "#B37420", position = ggplot2::position_nudge(x = nudge)) +
  # rcp 8.5 label
  ggplot2::stat_summary(data = cc_plants_rcp85, ggplot2::aes(x = scenario, y = change, label = paste0(round(..y.., 0), "%")), fun = median, geom = "text", colour = "#B37420", hjust = -0.5) +
  # rcp 8.5 iqr
  ggplot2::geom_errorbar(data = cc_plants_rcp85_iqr, ggplot2::aes(x = scenario, ymin = Q1, ymax = Q3), width = 0, colour = "#B37420", position = ggplot2::position_nudge(x = nudge)) +
  # add taxa icon
  rphylopic::add_phylopic(plant_icon, x = 0.65, y = 0.9 * max(cc_birds$change), ysize = 15) +
  ggplot2::scale_y_continuous(limits = c(0, max(cc_birds$change))) +
  ggplot2::labs(y = "Compositional\nchange (%)") +
  ggplot2::theme(axis.title.x = ggplot2::element_blank())

# butterflies violin plot
vio_butterflies <- ggplot2::ggplot() +
  # rcp 2.6 violin
  gghalves::geom_half_violin(data = cc_butterflies_rcp26, ggplot2::aes(x = scenario, y = change), colour = "#998ec3", fill = "#998ec3", alpha = 0.5, position = ggplot2::position_nudge(x = -nudge)) +
  # rcp 8.5 violin
  gghalves::geom_half_violin(data = cc_butterflies_rcp85, ggplot2::aes(x = scenario, y = change), side = "r", colour = "#f1a340", fill = "#f1a340", alpha = 0.5, position = ggplot2::position_nudge(x = nudge)) +
  # rcp 2.6 median
  ggplot2::stat_summary(data = cc_butterflies_rcp26, ggplot2::aes(x = scenario, y = change), fun = median, geom = "point", colour = "#69608C", position = ggplot2::position_nudge(x = -nudge)) +
  # rcp 2.6 label
  ggplot2::stat_summary(data = cc_butterflies_rcp26, ggplot2::aes(x = scenario, y = change, label = paste0(round(..y.., 0), "%")), fun = median, geom = "text", colour = "#69608C", hjust = rcp26_h) +
  # rcp 2.6 iqr
  ggplot2::geom_errorbar(data = cc_butterflies_rcp26_iqr, ggplot2::aes(x = scenario, ymin = Q1, ymax = Q3), width = 0, colour = "#69608C", position = ggplot2::position_nudge(x = -nudge)) +
    # rcp 8.5 median
  ggplot2::stat_summary(data = cc_butterflies_rcp85, ggplot2::aes(x = scenario, y = change), fun = median, geom = "point", colour = "#B37420", position = ggplot2::position_nudge(x = nudge)) +
  # rcp 8.5 label
  ggplot2::stat_summary(data = cc_butterflies_rcp85, ggplot2::aes(x = scenario, y = change, label = paste0(round(..y.., 0), "%")), fun = median, geom = "text", colour = "#B37420", hjust = -0.5) +
  # rcp 8.5 iqr
  ggplot2::geom_errorbar(data = cc_butterflies_rcp85_iqr, ggplot2::aes(x = scenario, ymin = Q1, ymax = Q3), width = 0, colour = "#B37420", position = ggplot2::position_nudge(x = nudge)) + 
  # add taxa icon
  rphylopic::add_phylopic(butter_icon, x = 0.65, y = 0.9 * max(cc_birds$change), ysize = 15) +
  ggplot2::scale_y_continuous(limits = c(0, max(cc_birds$change))) +
  ggplot2::labs(y = "Compositional\nchange (%)") +
  ggplot2::theme(axis.title.x = ggplot2::element_blank())

# birds violin plot
vio_birds <- ggplot2::ggplot() +
  # rcp 2.6 violin
  gghalves::geom_half_violin(data = cc_birds_rcp26, ggplot2::aes(x = scenario, y = change), colour = "#998ec3", fill = "#998ec3", alpha = 0.5, position = ggplot2::position_nudge(x = -nudge)) +
  # rcp 8.5 violin
  gghalves::geom_half_violin(data = cc_birds_rcp85, ggplot2::aes(x = scenario, y = change), side = "r", colour = "#f1a340", fill = "#f1a340", alpha = 0.5, position = ggplot2::position_nudge(x = nudge)) +
  # rcp 2.6 median
  ggplot2::stat_summary(data = cc_birds_rcp26, ggplot2::aes(x = scenario, y = change), fun = median, geom = "point", colour = "#69608C", position = ggplot2::position_nudge(x = -nudge)) +
  # rcp 2.6 label
  ggplot2::stat_summary(data = cc_birds_rcp26, ggplot2::aes(x = scenario, y = change, label = paste0(round(..y.., 0), "%")), fun = median, geom = "text", colour = "#69608C", hjust = rcp26_h) +
  # rcp 2.6 iqr
  ggplot2::geom_errorbar(data = cc_birds_rcp26_iqr, ggplot2::aes(x = scenario, ymin = Q1, ymax = Q3), width = 0, colour = "#69608C", position = ggplot2::position_nudge(x = -nudge)) +
  # rcp 8.5 median
  ggplot2::stat_summary(data = cc_birds_rcp85, ggplot2::aes(x = scenario, y = change), fun = median, geom = "point", colour = "#B37420", position = ggplot2::position_nudge(x = nudge)) +
  # rcp 8.5 label
  ggplot2::stat_summary(data = cc_birds_rcp85, ggplot2::aes(x = scenario, y = change, label = paste0(round(..y.., 0), "%")), fun = median, geom = "text", colour = "#B37420", hjust = -0.5) +
  # rcp 8.5 iqr
  ggplot2::geom_errorbar(data = cc_birds_rcp85_iqr, ggplot2::aes(x = scenario, ymin = Q1, ymax = Q3), width = 0, colour = "#B37420", position = ggplot2::position_nudge(x = nudge)) +
  # add taxa icon
  rphylopic::add_phylopic(bird_icon, x = 0.65, y = 0.9 * max(cc_birds$change), ysize = 15) +
  ggplot2::scale_y_continuous(limits = c(0, max(cc_birds$change))) +
  ggplot2::labs(y = "Compositional\nchange (%)") +
  ggplot2::theme(axis.title.x = ggplot2::element_blank())

# create legend for all plots
cc_leg <- cowplot::get_legend(ggplot2::ggplot(data.frame(category = c("RCP2.6", "RCP8.5"), value = c(10, 20)), aes(x = category, y = value, fill = category)) +
  ggplot2::geom_bar(stat = "identity") +
  ggplot2::scale_fill_manual(values = c("RCP2.6" = "#998ec3", "RCP8.5" = "#f1a340")) +
  ggplot2::theme(legend.title = ggplot2::element_blank()))

# combine plots
vio_comb <- cowplot::plot_grid(cowplot::plot_grid(vio_plants, vio_butterflies, vio_birds, nrow = 3, labels = "auto"), cc_leg, rel_widths = c(1, 0.15)) +
  ggplot2::theme(plot.background = element_rect(fill = "white", colour = "white"))

cowplot::save_plot("outputs/vio_comb.png", vio_comb, base_height = 8, base_width = 8, dpi = 300)
...
col_bi <- "DkViolet2"
# colours
# # biscale::bi_pal(pal = "DkViolet2", dim = 2, preview = FALSE)
...
coloc_map  <- function(df, rcp_name, tax_name, area = TRUE, icon = TRUE, title) {
  ...
  map <- ggplot2::ggplot(df_sub, ggplot2::aes(x = x, y = y, fill = bi_class)) +
    ...
    {if(area == TRUE) ggplot2::geom_text(x = 520000, y = 800000, label = paste0(sprintf("%.0f", area_dis), "%"), size = lab_size, colour = "#4279b0")} +
    # add area covered by novel bioclimates
    {if(area == TRUE) ggplot2::geom_text(x = 520000, y = 700000, label = paste0(sprintf("%.0f", area_nov), "%"), size = lab_size, colour = "#9e3547")} +
    # add taxa icon
...
# save
cowplot::save_plot(paste0("outputs/coloc_comb_min_thresh_", thresh, ".png"), coloc_comb, base_height = 9, base_width = 6.4, dpi = 600)
...
ext_plot <- function(df, icon = TRUE, leg = FALSE) {
  ...
  ext_plot_out <- ggplot2::ggplot(data = df, ggplot2::aes(x = year, y = pers_reg, colour = neat)) +
    ...
    ggplot2::scale_y_continuous(name = "Percent of\nspecies persisting", limits = c(pers_reg_min, pers_reg_max), sec.axis = ggplot2::sec_axis(trans = ~ ((100 - .) / 100) * df$n_spp, name = "Number of species\nheading for extinction")) +
...
# legend across plots
pmap_leg <- ggplot(data.frame(x = 1:10, y = 1:10, extinct = c(rep(lims[1], 9), lims[2])), aes(x = x, y = y, fill = extinct)) +
  geom_point(colour = "white") +
  ggplot2::labs(fill = "Percent of species\nheading for extinction") +
  scico::scale_fill_scico(palette = "bilbao", na.value = "grey", limits = lims, direction = -1, guide = guide_colorbar(title.position = "top", title.hjust = 0.5)) +
  theme_void() +
...
# Supplementary - Compositional change
...
# function to map compositional change due to climate change
cc_map <- function(gdmr_tax, covars_rcp, title, lims, leg = FALSE) {
  ...
  cc_out <- ggplot2::ggplot(dplyr::filter(timep_df, !is.na(change)), aes(x = x, y = y, fill = change)) +
    ...
    ggplot2::coord_equal() +
    ggplot2::theme_void() +
    ggplot2::labs(title = title, fill = "Compositional\nchange (%)") +
...
# RCP 2.6 2030
ccmap_rcp26_2030 <- cc_map(gdmr_tax = gdmr_tax, covars_rcp = raster::stack(covars_rcp26_2030), title = "RCP2.6\n2030", lims = lims)

# RCP 2.6 2050
ccmap_rcp26_2050 <- cc_map(gdmr_tax = gdmr_tax, covars_rcp = raster::stack(covars_rcp26_2050), title = "RCP2.6\n2050", lims = lims)

# RCP 2.6 2070
ccmap_rcp26_2070 <- cc_map(gdmr_tax = gdmr_tax, covars_rcp = raster::stack(covars_rcp26_2070), title = "RCP2.6\n2070", lims = lims)

# RCP 4.5 2030
ccmap_rcp45_2030 <- cc_map(gdmr_tax = gdmr_tax, covars_rcp = raster::stack(covars_rcp45_2030), title = "RCP4.5\n2030", lims = lims)

# RCP 4.5 2050
ccmap_rcp45_2050 <- cc_map(gdmr_tax = gdmr_tax, covars_rcp = raster::stack(covars_rcp45_2050), title = "RCP4.5\n2050", lims = lims)

# RCP 4.5 2070
ccmap_rcp45_2070 <- cc_map(gdmr_tax = gdmr_tax, covars_rcp = raster::stack(covars_rcp45_2070), title = "RCP4.5\n2070", lims = lims)

# RCP 6.0 2030
ccmap_rcp60_2030 <- cc_map(gdmr_tax = gdmr_tax, covars_rcp = raster::stack(covars_rcp60_2030), title = "RCP6.0\n2030", lims = lims)

# RCP 6.0 2050
ccmap_rcp60_2050 <- cc_map(gdmr_tax = gdmr_tax, covars_rcp = raster::stack(covars_rcp60_2050), title = "RCP6.0\n2050", lims = lims)

# RCP 6.0 2070
ccmap_rcp60_2070 <- cc_map(gdmr_tax = gdmr_tax, covars_rcp = raster::stack(covars_rcp60_2070), title = "RCP6.0\n2070", lims = lims)

# RCP 8.5 2030
ccmap_rcp85_2030 <- cc_map(gdmr_tax = gdmr_tax, covars_rcp = raster::stack(covars_rcp85_2030), title = "RCP8.5\n2030", lims = lims)

# RCP 8.5 2050
ccmap_rcp85_2050 <- cc_map(gdmr_tax = gdmr_tax, covars_rcp = raster::stack(covars_rcp85_2050), title = "RCP8.5\n2050", lims = lims)

# RCP 8.5 2070
ccmap_rcp85_2070 <- cc_map(gdmr_tax = gdmr_tax, covars_rcp = raster::stack(covars_rcp85_2070), title = "RCP8.5\n2070", lims = lims)
...
# save
cowplot::save_plot(paste0("outputs/", tax, "/", tax, "_biochange_maps_comb.png"), cc_maps_comb, base_height = 10, base_width = 6, dpi = 600)
...
disappear_map_out <- function(scenario_name, metric, title, lims, tax, leg = FALSE) {
  ...
  d_map <- ggplot2::ggplot(disappear_df, ggplot2::aes(x = x, y = y, fill = !!metric)) +
    ...
    # colours
    ggplot2::scale_fill_gradientn(colours = c("#EBF2F8", "#B1C9E2", "#78A1CA", "#4279b0", "#38699B", "#2F5A85", "#264A6F", "#1D3B59", "#152B42", "#0D1C2A", "#060C13"), limits = lims) + 
    # theme
    ggplot2::theme_void() +
...
# save
cowplot::save_plot(paste0("outputs/", tax, "/", tax, "_d_maps_comb.png"), d_maps_comb, base_height = 10, base_width = 6.2, dpi = 600)
...
novel_map_out <- function(scenario_name, metric, med = FALSE, title, lims, tax, leg = FALSE) {
  ...
  n_map <- ggplot2::ggplot(novel_df, ggplot2::aes(x = x, y = y, fill = !!metric)) +
    ...
    # colours
    ggplot2::scale_fill_gradientn(colours = c("#F6EAEC", "#DBABB4", "#BE6F7D", "#9e3547", "#8B2D3D", "#782533", "#641E2A", "#501721", "#3B1018", "#260A0F", "#110406"), limits = lims) +
    # theme
    ggplot2::theme_void() +
    {if(leg == FALSE) ggplot2::theme(legend.position = "none", plot.margin = unit(c(0, 0, 0, 0), "cm"))} +
    ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5, face = "bold"))
...
# save
cowplot::save_plot(paste0("outputs/", tax, "/", tax, "_n_maps_comb.png"), n_maps_comb, base_height = 10, base_width = 6.2, dpi = 600)
...
if(tax == "plants") gdmr_tax <- gdmr_plants
if(tax == "butterflies") gdmr_tax <- gdmr_butterflies
if(tax == "birds") gdmr_tax <- gdmr_birds; class(gdmr_tax) = "gdm"
...
# gdmTab_birds is over 100Mb so isn't supported by GitHub, ask author for a copy
